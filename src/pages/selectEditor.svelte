...
>
  ...
  {#if !isHasBasic && !isTeamMember}
    <div class="flex text-black flex-col justify-between h-full">
      <div>
        ...
        <h1 class="text-sm text-neutral-700">
          Please activate your account using Qubic product.
        </h1>
        <p>
          Need help? <button
            class="font-bold"
  ...
  {#if isHasBasic}
    <button
      on:click={() => {
        if ($user?.confirmed_at) goto("/basic/profile");
        else toastFailed("Please confirm your email first");
      }}
      transition:slide|local={{ duration: 500 }}
      class="flex items-center justify-between p-3 md:p-4 mb-3 rounded-md w-full cursor-pointer bg-blue-600 text-white transition-colors duration-200"
    >
      <p>Basic Editor</p>
    </button>
  {/if}
    ...
    {#if teams}
      {#if teams.length > 0}
        {#each teams as item}
          ...
          >
            <img
              src={item.metadata.logo != null &&
              item.metadata.logo.toString().length > 0
                ? item.metadata.logo
                : "/favicon.svg"}
              alt=""
