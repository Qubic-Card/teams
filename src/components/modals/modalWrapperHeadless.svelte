...
<Dialog
  ...
  class={`transition-all md:transition-none duration-300 ease-in-out ${
    isOpen
      ? `${mobileHeight} ${desktopHeight}`
      : 'h-0 translate-y-10 md:opacity-0'
  } flex flex-col ml-0 lg:ml-12 w-full ${text} ${desktopWidth} md:max-w-md px-4 pb-4 gap-4 bottom-0 fixed ${desktopTop} ${desktopRight} lg:left-1/3 z-50  border-0 shadow-lg rounded-lg ${bg} outline-none focus:outline-none overflow-y-auto snap-container`}
>
  <slot />
</Dialog>
