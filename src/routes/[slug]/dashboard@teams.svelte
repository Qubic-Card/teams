<div class="flex flex-col lg:flex-row justify-between text-white">
  <div
    class="flex-1 flex flex-col items-start justify-between bg-neutral-800 lg:mr-4 p-8 text-2xl md:text-4xl lg:text-5xl font-bold h-56 mb-4 lg:mb-0 lg:h-80"
  >
    <h1>Tahesta</h1>
    <div>
      <p>5 Total Members</p>
      <p>25 Total Connections</p>
    </div>
  </div>
  <div
    class="flex-1 flex flex-col items-start justify-between bg-blue-700 p-8 text-2xl md:text-4xl lg:text-5xl font-bold h-56 lg:h-80"
  >
    <h1>Tahesta</h1>
    <div>
      <p>5 Total Members</p>
      <p>25 Total Connections</p>
    </div>
  </div>
</div>
<div class="flex mt-4">
  <div class="flex-1 flex flex-col text-black mr-4">
    <div
      class="flex flex-col text-neutral-100 bg-neutral-800 justify-center p-8 pl-8 lg:pl-16 h-auto mb-4"
    >
      <h1 class="text-xl lg:text-2xl font-bold mb-1">Axel</h1>
      <p>Connected with Nelson Manager at POPL</p>
    </div>
    <div
      class="flex flex-col text-neutral-100 bg-neutral-800 justify-center p-8 pl-8 lg:pl-16 h-auto mb-4"
    >
      <h1 class="text-xl lg:text-2xl font-bold mb-1">Axel</h1>
      <p>Connected with Nelson Manager at POPL</p>
    </div>
    <div
      class="flex flex-col text-neutral-100 bg-neutral-800 justify-center p-8 pl-8 lg:pl-16 h-auto mb-4"
    >
      <h1 class="text-xl lg:text-2xl font-bold mb-1">Axel</h1>
      <p>Connected with Nelson Manager at POPL</p>
    </div>
  </div>
  <div
    class="text-neutral-100 bg-neutral-800 w-[200px] md:w-[300px] lg:w-[700px]"
  >
    No Content
  </div>
</div>
